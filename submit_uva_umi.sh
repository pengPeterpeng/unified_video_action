#!/bin/bash
#SBATCH --job-name=uva_umi_multi
#SBATCH --nodes=1
#SBATCH --ntasks=8                  # 启动 n 个任务（等于进程数）
#SBATCH --gpus=8                    # 请求 n 块 GPU
#SBATCH --cpus-per-task=16          # 每个进程分配 16 个 CPU（根据模型大小和数据处理需求调整）
#SBATCH --mem=256G                  # 总内存（也可使用 --mem-per-cpu）
...
accelerate launch --num_processes=8 train.py \
    ...
    hydra.run.dir="checkpoints/uva_umi_cup_video_epoch"